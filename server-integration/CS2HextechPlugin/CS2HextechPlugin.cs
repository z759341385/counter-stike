    ...
    public class CS2HextechPlugin : BasePlugin
    {
        ...
        public override void Load(bool hotReload)
        {
            ...
            if (hotReload)
            {
                ...
                if (gameRules != null)
                {
                    _isGameStarted = !gameRules.WarmupPeriod;
                }
            }
        }
        ...
        private int GetCurrentRound()
        {
            ...
            if (gameRules != null)
            {
                if (gameRules.WarmupPeriod) return 0;
                return gameRules.TotalRoundsPlayed + 1;
            }
            return 1;
        }
        ...
        private HookResult OnTakeDamage(CEntityInstance entity, CTakeDamageInfo info)
        {
            ...
            var victimPlayer = victimPawn.Controller.Value;
            ...
            // 2. 攻击者相关效果
            if (attackerEntity is CCSPlayerPawn attackerPawn)
            {
                var attackerPlayer = attackerPawn.Controller.Value;
                if (attackerPlayer != null && attackerPlayer.IsValid && !attackerPlayer.IsBot)
                {
                    // 玻璃大炮 (伤害提升 50%)
        ...
        private HookResult OnRoundStart(EventRoundStart @event, GameEventInfo info)
        {
            ...
            // 如果游戏未开启，确保强制保持在热身状态
            var gameRules = Utilities.FindAllEntitiesByDesignerName<CCSGameRules>("cs_gamerules").FirstOrDefault();
            if (gameRules != null && !gameRules.WarmupPeriod && !_isGameStarted)
            {
                Server.ExecuteCommand("mp_warmuptime 9999");
                Server.ExecuteCommand("mp_warmup_start");
            }
