  ...
  socket.on('start_hextech_game', ({ roomId }, callback) => {
    ...
    if (room) {
      ...
      if (host) {
        ...
        if (typeof callback === 'function') callback({ success: true, server: serverInfo });
        io.to(roomId).emit('hextech_server_ready', serverInfo);
        logger.info('Room', `房间 ${roomId} 海克斯模式开始, 分配服务器: ${serverInfo.ip}:${serverInfo.port}`);
      } else {
        if (typeof callback === 'function') callback({ success: false, error: '仅房主可开始游戏' });
      }
    } else {
